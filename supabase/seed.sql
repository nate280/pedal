...
begin
  ...
  insert into public.issues (bike_id, description, severity, status, reported_by, created_at, resolved_at) values
    ...
    ((select id from public.bikes where studio_id = sid and name = 'Spin 12'),
     'Belt tension loose, retensioned.',
     'high', 'resolved', 'Maya R.', now() - interval '18 days', now() - interval '17 days'),
    ((select id from public.bikes where studio_id = sid and name = 'Spin 06'),
    ...
    ((select id from public.bikes where studio_id = sid and name = 'Spin 08'),
    ...
    ((select id from public.bikes where studio_id = sid and name = 'Spin 13'),
    ...
    ((select id from public.bikes where studio_id = sid and name = 'Spin 04'),
     'Toe cage replaced.',
  ...
  where b.studio_id = sid and b.name = 'Spin 02' and i.status = 'in_progress';

  insert into public.issue_notes (issue_id, note, created_by)
  select i.id, 'Confirmed grinding originates from drive side. Bike tagged out of service.', 'Maya R.'
  from public.issues i
  join public.bikes b on b.id = i.bike_id
  where b.studio_id = sid and b.name = 'Spin 03' and i.status = 'open';

  insert into public.issue_notes (issue_id, note, created_by)
  select i.id, 'New straps in stock, swapping today.', 'Sam T.'
  from public.issues i
  join public.bikes b on b.id = i.bike_id
  where b.studio_id = sid and b.name = 'Spin 11' and i.status = 'in_progress';

  raise notice 'Pedal seed complete: 16 bikes, 18 issues.';
end $$;
